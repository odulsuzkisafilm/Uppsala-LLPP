...
int main(int argc, char*argv[]) {
	...
	while (i < argc) {
		if (argv[i][0] == '-' && argv[i][1] == '-') {
			if (strcmp(&argv[i][2], "timing-mode") == 0) {
				...
				cout << "Using collision detection!" << endl;
				impl = Ped::SEQ_COL;
			}
			else if (strcmp(&argv[i][2], "nthreads") == 0) {
				i += 1;
				if (i >= argc) {
					cerr << "Argument --nthreads requires an integer: " << argv[i] << endl;
					return -1;
				}

				std::istringstream ss(argv[i]);
				if (!(ss >> nthreads)) {
					cerr << "Invalid number of threads: " << argv[i] << endl;
					return -1;
				}
				cout << "Set nthreads to " << nthreads << endl;
			}
			...
			else if (strcmp(&argv[i][2], "omp-col") == 0) {
				cout << "Using OpenMP with collision detection!" << endl;
				impl = Ped::OMP_COL;
			}
			else if (strcmp(&argv[i][2], "vector") == 0) {
				cout << "Using Vectorization!" << endl;
				impl = Ped::VECTOR;
			}
			...
			else if (strcmp(&argv[i][2], "tick-count") == 0) {
				i += 1;
				if (i >= argc) {
					cerr << "Argument --tick-count requires an integer: " << argv[i] << endl;
					return -1;
				}

				std::istringstream ss(argv[i]);
				if (!(ss >> tick_count)) {
					cerr << "Invalid tick count: " << argv[i] << endl;
					return -1;
				}
				cout << "Set tick count to " << tick_count << endl;
			}
			else {
	...
	{ // This scope is for the purpose of removing false memory leak positives
		...
		model.setup(parser.getAgents(), parser.getWaypoints(), impl, nthreads);

		// Default number of steps to simulate. Feel free to change this.
		// const int maxNumberOfStepsToSimulate = 100000;
		const int maxNumberOfStepsToSimulate = tick_count;
		...
		if (timing_mode)
		{
			...
			Ped::IMPLEMENTATION impl_ref = (impl == Ped::OMP_COL) ? Ped::SEQ_COL : Ped::SEQ;
			double fps_seq, fps_target;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
				model.setup(parser.getAgents(), parser.getWaypoints(), impl_ref, 0);
			...
			Ped::IMPLEMENTATION impl_to_test = impl;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
				model.setup(parser.getAgents(), parser.getWaypoints(), impl_to_test, nthreads);
				PedSimulation simulation(model, NULL, timing_mode);
				// Simulation mode to use when profiling (without any GUI)
