...
ViewAgent::ViewAgent(Ped::Tagent * agent, QGraphicsScene * scene, size_t index) : agent(agent), index(index)
{
...
/* What do you call a small bug? */
	...
	bgt_icon->setPos(MainWindow::cellToPixel(agent->getX(index)), MainWindow::cellToPixel(agent->getY(index)));
#else
	bgt_icon = 0;
	rect = scene->addRect(MainWindow::cellToPixel(agent->getX(index)), MainWindow::cellToPixel(agent->getY(index)),
...
void ViewAgent::paint(QColor color){
	...
	else
	{
		QBrush brush(color);
		rect->setBrush(brush);
		rect->setRect(MainWindow::cellToPixel(agent->getX(index)), MainWindow::cellToPixel(agent->getY(index)),
...
const std::pair<int, int> ViewAgent::getPosition(){
	return std::make_pair(agent->getX(index), agent->getY(index));
}
