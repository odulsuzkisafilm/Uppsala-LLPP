...
MainWindow::MainWindow(const Ped::Model &pedModel) : model(pedModel)
{
	...
	if (model.getImplementation() == Ped::VECTOR) {
		for (size_t index = 0; index < agents[0]->num_agents; index++)
		{
			viewAgents.push_back(new ViewAgent(agents[0], scene, index));
		}
	}
	else {
		...
		{
			viewAgents.push_back(new ViewAgent(*it, scene, 0));
		}
	}
